...
export class PackageBundler {
  ...
  private async createRollupBundle(config: RollupBundleConfig) {
    ...
    if (buildConfig.licenseBanner) {
      bundleOptions.plugins.push(rollupRemoveLicensesPlugin);
    }

    const writeOptions: any = {
      // Keep the moduleId empty because we don't want to force developers to a specific moduleId.
      moduleId: '',
      moduleName: config.moduleName || `ng.${buildConfig.packageName}`,
    ...
    if (buildConfig.licenseBanner) {
      writeOptions.banner = buildConfig.licenseBanner;
    }
