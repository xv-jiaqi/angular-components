...
import { buildLessTask } from './build-less-task';
...
export function createPackageBuildTasks(buildPackage: BuildPackage) {
  ...
  task(`${taskName}:assets`, [
    `${taskName}:assets:less`,
    `${taskName}:assets:es5-less`,
    `${taskName}:assets:copy-styles`,
    `${taskName}:assets:html`
  ]);

  task(`${taskName}:assets:less`, buildLessTask(
    buildPackage.outputDir, buildPackage.sourceDir, true)
  );

  task(`${taskName}:assets:es5-less`, buildLessTask(
  ...
  /**
   * Watch tasks, that will rebuild the package whenever TS, LESS, or HTML files change.
   */
  task(`${taskName}:watch`, dependentWatchTasks, () => {
    watchFiles(join(buildPackage.sourceDir, '**/*.+(ts|less|html)'), [`${taskName}:build`]);
  });
}
