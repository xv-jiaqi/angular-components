...
export function inlineMetadataResources(metadata: any, componentResources: Map<string, string>) {
  ...
  if (metadata.styleUrls && metadata.styleUrls.length) {
    ...
    for (const styleUrl of metadata.styleUrls) {
      const replacedStyleUrl = styleUrl.replace(/\.(scss|less|sass)$/, '.css')
      const fullResourcePath = componentResources.get(basename(replacedStyleUrl));
      if (!fullResourcePath) {
        throw new Error(`can not get path from ${replacedStyleUrl}`);
      }
...
export function inlinePackageMetadataFiles(packagePath: string) {
  ...
  glob(join(packagePath, '**/*.+(html|css)')).forEach(resourcePath => {
    const replacedPath = resourcePath.replace(/\.(scss|less|sass)$/, '.css');
    componentResources.set(basename(replacedPath), replacedPath);
  });
