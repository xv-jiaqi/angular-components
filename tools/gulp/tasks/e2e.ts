...
import {buildConfig, sequenceTask, watchFiles} from 'build-tools';
...
task('e2e-app:build', sequenceTask(
  'clean',
  [
    `${buildConfig.packageName}:build-release`
  ],
...
task('e2e-app:copy-release', () => {
  copySync(join(releasesDir, buildConfig.packageName), join(outDir, buildConfig.packageName));
});
